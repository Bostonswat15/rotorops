 ...
 * 20260914000000_maintenance_and_loans.sql. These only preview them so the
 * Maintenance page can say what a service will do before you pay for it. If
 * the two ever drift, the server's number is the one that counts.
 */
...
export function serviceQuote(a: Airframe, type: ServiceType): ServiceQuote {
  ...
  const removed = Math.min(wear, type === "inspection" ? 30 : type === "overhaul" ? 80 : 0);
