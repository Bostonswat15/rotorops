-- =============================================================================
-- Maintenance that matters, resale that falls with use, and loans.
--
-- Maintenance
--   * 100-hour inspections. hours_at_inspection records airframe hours at the
--     last inspection or overhaul; more than 10 hours past due and the aircraft
--     can't be dispatched. The existing fleet starts on a fresh clock.
--   * Wear costs money: above 40% wear, operating cost rises 0.5% a point.
--   * Breakdowns finally use reliability. Each flight rolls
--     wear x (100 - reliability) x 25%. A breakdown adds 20 wear and grounds the
--     aircraft until it is repaired (1% of its price). It never interrupts the
--     flight or fails the contract -- it is decided when the flight is logged.
--   * Servicing no longer marks an aircraft available while it is still
--     broken down or above the grounding wear.
--
-- Resale
--   price x 70% x 0.8^(hours / 500) x (1 - 15% x wear), floor 10%. Hours keep
--   counting past 2,000, and an overhaul right before selling wins back at most
--   ~12% of the price instead of ~24%.
--
-- Loans
--   Credit limit $100k + half the resale value of owned aircraft. A 5% fee is
--   charged when you borrow; 10% of every contract payout repays the balance
--   automatically; repay early any time. No interest over time: money in this
--   game only moves when someone flies, so a timed charge would grow while
--   nobody is in the sim.
--
-- rotorops_resolve_flight is carried forward from 20260910000000_pilot_skills.sql
-- with the wear cost, the breakdown roll and loan repayment added. Everything
-- else in it is unchanged -- CREATE OR REPLACE takes the whole body.
--
-- The new columns are deliberately absent from the client UPDATE grants in
-- 20260827120200_column_grants.sql, so neither a loan balance nor an inspection
-- clock can be edited from the app.
--
-- Safe to re-run.
-- =============================================================================
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns
                  WHERE table_schema = 'public' AND table_name = 'aircraft'
                    AND column_name = 'hours_at_inspection') THEN
    ALTER TABLE public.aircraft ADD COLUMN hours_at_inspection NUMERIC NOT NULL DEFAULT 0;
    -- A fresh clock for the existing fleet, not a fleet that is instantly overdue.
    UPDATE public.aircraft SET hours_at_inspection = hours;
  END IF;
END $$;

ALTER TABLE public.aircraft ADD COLUMN IF NOT EXISTS broken_down_at TIMESTAMPTZ;
ALTER TABLE public.companies ADD COLUMN IF NOT EXISTS loan_balance NUMERIC NOT NULL DEFAULT 0;

-- --------------------------------------------------------------------------
-- Resale value
-- --------------------------------------------------------------------------

-- The formula on its own, so the sale price, the credit limit and the balance
-- sheet can't drift apart. Mirrored for display in src/lib/maintenance.ts.
CREATE OR REPLACE FUNCTION public.aircraft_resale(
  _acquisition_cost NUMERIC, _hours NUMERIC, _wear NUMERIC)
RETURNS NUMERIC LANGUAGE sql IMMUTABLE AS $fn$
  SELECT ROUND(GREATEST(COALESCE(_acquisition_cost, 0), 0) * GREATEST(
    0.70
      * power(0.8, GREATEST(COALESCE(_hours, 0), 0) / 500.0)
      * (1 - 0.15 * LEAST(GREATEST(COALESCE(_wear, 0), 0), 100) / 100.0),
    0.10));
$fn$;
...
-- --------------------------------------------------------------------------
-- Dispatch: an aircraft more than 10 hours past its inspection stays home.
-- Carried forward from 20260827120100_coop_economy.sql with that one check.
-- --------------------------------------------------------------------------
...
-- --------------------------------------------------------------------------
-- Servicing: inspection, overhaul, and now repair after a breakdown.
-- --------------------------------------------------------------------------
...
BEGIN
  ...
  END IF;
  IF _type = 'repair' AND v_ac.broken_down_at IS NULL THEN
  ...
  v_cost := CASE _type
    ...
    ELSE ROUND(v_ac.acquisition_cost * 0.01)
  END;
  ...
  v_new_wear := GREATEST(0, v_ac.wear - v_wear);
...
-- --------------------------------------------------------------------------
-- Flight resolution: wear cost, breakdown roll, loan repayment.
-- Carried forward from 20260910000000_pilot_skills.sql.
-- --------------------------------------------------------------------------
...
-- --------------------------------------------------------------------------
-- Loans and the balance sheet
-- --------------------------------------------------------------------------

-- $100k plus half of what the owned fleet would sell for. Internal: the app
-- reads it through company_balance_sheet, which checks membership.
...
RETURNS NUMERIC LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $fn$
  SELECT 100000 + 0.5 * COALESCE(SUM(public.aircraft_resale(acquisition_cost, hours, wear)), 0)
...
BEGIN
  ...
  RETURN jsonb_build_object(
    ...
    'assets', v_co.cash + v_fleet,
    'loan_balance', v_co.loan_balance,
    'liabilities', v_co.loan_balance,
    'company_value', v_co.cash + v_fleet - v_co.loan_balance,
    'loan_limit', v_limit,
    'available_credit', GREATEST(0, v_limit - v_co.loan_balance));
END;$fn$;

CREATE OR REPLACE FUNCTION public.take_loan(_company_id UUID, _amount NUMERIC)
RETURNS JSONB LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $fn$
DECLARE
  v_co     public.companies%ROWTYPE;
  v_amount NUMERIC := ROUND(COALESCE(_amount, 0));
  v_limit  NUMERIC;
  v_fee    NUMERIC;
BEGIN
  IF NOT public.can_manage_company(_company_id) THEN
    RAISE EXCEPTION 'only owners and managers can borrow';
  END IF;
  IF v_amount <= 0 THEN RAISE EXCEPTION 'enter an amount to borrow'; END IF;

  SELECT * INTO v_co FROM public.companies WHERE id = _company_id FOR UPDATE;
  IF NOT FOUND THEN RAISE EXCEPTION 'company not found'; END IF;

  v_limit := public.company_loan_limit(_company_id);
  IF v_co.loan_balance + v_amount > v_limit THEN
    RAISE EXCEPTION 'that is over your credit limit -- $% available',
      ROUND(GREATEST(0, v_limit - v_co.loan_balance));
  END IF;

  v_fee := ROUND(v_amount * 0.05);
  UPDATE public.companies
     SET cash = cash + v_amount - v_fee,
         loan_balance = loan_balance + v_amount
   WHERE id = _company_id;
  INSERT INTO public.economy_transactions (company_id, type, amount, description) VALUES
    (_company_id, 'loan_draw', v_amount, 'Loan drawn'),
    (_company_id, 'loan_fee', -v_fee, format('Loan fee: 5%% of $%s', v_amount));

  RETURN jsonb_build_object(
    'borrowed', v_amount,
    'fee', v_fee,
    'loan_balance', v_co.loan_balance + v_amount);
END;$fn$;

CREATE OR REPLACE FUNCTION public.repay_loan(_company_id UUID, _amount NUMERIC)
RETURNS JSONB LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $fn$
DECLARE
  v_co     public.companies%ROWTYPE;
  v_amount NUMERIC;
BEGIN
  IF NOT public.can_manage_company(_company_id) THEN
    RAISE EXCEPTION 'only owners and managers can repay the loan';
  END IF;

  SELECT * INTO v_co FROM public.companies WHERE id = _company_id FOR UPDATE;
  IF NOT FOUND THEN RAISE EXCEPTION 'company not found'; END IF;

  v_amount := LEAST(ROUND(COALESCE(_amount, 0)), v_co.loan_balance);
  IF v_amount <= 0 THEN RAISE EXCEPTION 'nothing to repay'; END IF;
  IF v_co.cash < v_amount THEN RAISE EXCEPTION 'insufficient cash'; END IF;
  ...
  UPDATE public.companies
   ...
   WHERE id = _company_id;
  INSERT INTO public.economy_transactions (company_id, type, amount, description)
  VALUES (_company_id, 'loan_repayment', -v_amount, 'Loan repaid early');
...
-- Grants
-- --------------------------------------------------------------------------
GRANT EXECUTE ON FUNCTION public.aircraft_resale(NUMERIC, NUMERIC, NUMERIC) TO authenticated;
...
GRANT EXECUTE ON FUNCTION public.take_loan(UUID, NUMERIC) TO authenticated;
GRANT EXECUTE ON FUNCTION public.repay_loan(UUID, NUMERIC) TO authenticated;

REVOKE EXECUTE ON FUNCTION public.aircraft_resale(NUMERIC, NUMERIC, NUMERIC) FROM PUBLIC, anon;
...
REVOKE EXECUTE ON FUNCTION public.take_loan(UUID, NUMERIC) FROM PUBLIC, anon;
REVOKE EXECUTE ON FUNCTION public.repay_loan(UUID, NUMERIC) FROM PUBLIC, anon;
REVOKE EXECUTE ON FUNCTION public.company_loan_limit(UUID) FROM PUBLIC, anon, authenticated;
