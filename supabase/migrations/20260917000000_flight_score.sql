...
-- Flight score.
--
-- The bridge scores every flight out of 100 (bridge/src/score.ts): the worst
-- touchdown by rate and G, lights, overspeed and stall warnings, bank, pitch and
-- G, engine and sling incidents, the fuel reserve on landing, and small bonuses
-- for night and low-visibility landings. This stores it and lets the grade
-- price the flight:
--
--   grade   score    pay     XP      reputation
--   A       90-100   +5%     +10%    +1
--   B       75-89    --      --      --
--   C       60-74    -5%     -10%    --
--   D       40-59    -10%    -25%    --
--   F       0-39     -20%    -50%    -1
--
-- The grade replaces the old landing-quality pay adjustment (x1.05 excellent,
-- x0.9 hard) and its reputation bonus. A severe landing or a crash still fails
-- or resets the contract as before. A flight with no score -- a manual log, or
-- a bridge that predates this -- is priced by landing quality exactly as it
-- always was.
--
-- The score comes from the bridge, the same trust already given to its touchdown
-- rate; it is clamped to 0-100 here.
--
-- rotorops_resolve_flight is carried forward from 20260916000000_crash_restart.sql
-- with the score added. company_roster gains each pilot's average score; its
-- result columns change, so it is dropped and recreated.
--
-- Run 20260913 through 20260916 first. Safe to re-run.
-- =============================================================================

ALTER TABLE public.flight_logs
  ADD COLUMN IF NOT EXISTS score INTEGER CHECK (score BETWEEN 0 AND 100),
  ADD COLUMN IF NOT EXISTS grade TEXT,
  ADD COLUMN IF NOT EXISTS score_items JSONB NOT NULL DEFAULT '[]'::JSONB;

-- --------------------------------------------------------------------------
-- Crew roster with average score.
-- --------------------------------------------------------------------------
DROP FUNCTION IF EXISTS public.company_roster(UUID);
CREATE FUNCTION public.company_roster(_company_id UUID)
RETURNS TABLE (user_id UUID, display_name TEXT, role TEXT, callsign TEXT,
               joined_at TIMESTAMPTZ, flights BIGINT, hours NUMERIC, avg_score NUMERIC)
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $fn$
  SELECT cm.user_id, p.display_name, cm.role, cm.callsign, cm.joined_at,
         COUNT(fl.id),
         COALESCE(SUM(fl.duration_hr), 0),
         ROUND(AVG(fl.score), 1)
...
-- --------------------------------------------------------------------------
-- Flight resolution: the grade prices the flight.
