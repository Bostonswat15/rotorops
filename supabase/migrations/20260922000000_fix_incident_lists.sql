-- =============================================================================
-- Flights with an incident could not be logged.
--
-- rotorops_resolve_flight added incidents with `v_incidents || 'hard landing'`.
-- v_incidents is TEXT[], and Postgres types an untyped literal beside an array
-- as another array, so it tries to parse 'hard landing' as an array literal
-- and raises "malformed array literal". Every flight that recorded an
-- off-contract landing, hard landing, skid damage, crash damage or breakdown
-- failed to submit, and the bridge does not retry. Seen 2026-09-14: a
-- Powerline Patrol with every objective done, landed at -303 fpm, rejected
-- with 'malformed array literal: "hard landing"'.
--
-- Those five lines now use array_append. Nothing else in the function changes.
-- The not-restarted line was already fine: format() returns text.
--
-- Carried forward whole from 20260919000000_pilot_ratings.sql. Any later change
-- to rotorops_resolve_flight must start from this copy. Re-runnable.
-- =============================================================================
